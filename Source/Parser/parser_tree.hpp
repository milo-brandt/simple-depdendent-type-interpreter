...
inline void path_append(parser::path::Path& path, Tree Apply::* member) {
  if(member == &Apply::lhs) {
    path.steps.push_back(0);
    return;
  }
  if(member == &Apply::rhs) {
    path.steps.push_back(1);
    return;
  }
  std::terminate(); //unreachable
}
inline void path_append(parser::path::Path& path, Tree Lambda::* member) {
  if(member == &Lambda::body) {
    path.steps.push_back(0);
    return;
  }
  std::terminate(); //unreachable
}
inline void path_append(parser::path::Path& path, std::optional<Tree> Lambda::* member) {
  if(member == &Lambda::type) {
    path.steps.push_back(1);
    return;
  }
  std::terminate(); //unreachable
}
inline void path_append(parser::path::Path& path, Tree Arrow::* member) {
  if(member == &Arrow::domain) {
    path.steps.push_back(0);
    return;
  }
  if(member == &Arrow::codomain) {
    path.steps.push_back(1);
    return;
  }
  std::terminate(); //unreachable
...
inline void path_append(parser::path::Path& path, T1&& t1, T2&& t2, Ts&&... ts) {
  path_append(path, std::forward<T1>(t1));
  path_append(path, std::forward<T2>(t2), std::forward<Ts>(ts)...);
}

  
  inline Tree& lookup(Tree& in, parser::path::Path const& path) {
    Tree* ret = &in;
    for(auto step : path.steps) {
      ret = ret->visit([&]<class T>(T& part) -> Tree* {
  ...
  inline Tree const& lookup(Tree const& in, parser::path::Path const& path) {
    Tree const* ret = &in;
    for(auto step : path.steps) {
      ret = ret->visit([&]<class T>(T const& part) -> Tree const* {
        if constexpr(std::is_same_v<T, Apply>) {
          switch(step) {
          case 0: return &part.lhs;
          case 1: return &part.rhs;
...
namespace parser::located_output {
  struct Apply;
  struct Lambda;
  struct Identifier;
  struct Hole;
  ...
  Tree::Tree(Apply arg):
    output([&]() -> parser::output::Tree {
      return parser::output::Apply{
        .lhs = std::move(arg.lhs.output),
        .rhs = std::move(arg.rhs.output)
      };
    }()),
    locator([&]() -> parser::locator::Tree {
      return parser::locator::Apply{
  ...
  Tree::Tree(Lambda arg):
    output([&]() -> parser::output::Tree {
      return parser::output::Lambda{
        .body = std::move(arg.body.output),
        .type = [&]() -> std::optional<parser::output::Tree> {
          if(arg.type) {
    ...
    locator([&]() -> parser::locator::Tree {
      return parser::locator::Lambda{
        .body = std::move(arg.body.locator),
        .type = [&]() -> std::optional<parser::locator::Tree> {
          if(arg.type) {
  ...
  Tree::Tree(Identifier arg):
    output([&]() -> parser::output::Tree {
      return parser::output::Identifier{
        .id = std::move(arg.id)
      };
    }()),
    locator([&]() -> parser::locator::Tree {
      return parser::locator::Identifier{
        .position = std::move(arg.position)
      };
    }())
  {}
  Tree::Tree(Hole arg):
    output([&]() -> parser::output::Tree {
      return parser::output::Hole{
      };
    }()),
    locator([&]() -> parser::locator::Tree {
      return parser::locator::Hole{
        .position = std::move(arg.position)
      };
    }())
  {}
