...
namespace expression_parser::output {
	...
	inline Tree::Tree(Identifier arg):data(mdb::in_place, std::move(arg)) {}
	inline Tree::Tree(Hole arg):data(mdb::in_place, std::move(arg)) {}
	inline Tree::Tree(Arrow arg):data(mdb::in_place, std::move(arg)) {}
	...
	inline Identifier& Tree::get_identifier() & { return std::get<Identifier>(data->data); }
	inline Identifier const& Tree::get_identifier() const& { return std::get<Identifier>(data->data); }
	inline Identifier&& Tree::get_identifier() && { return std::move(std::get<Identifier>(data->data)); }
	inline Identifier* Tree::get_if_identifier() { return std::get_if<Identifier>(&data->data); }
	inline Identifier const* Tree::get_if_identifier() const { return std::get_if<Identifier>(&data->data); }
	...
	inline Arrow& Tree::get_arrow() & { return std::get<Arrow>(data->data); }
	inline Arrow const& Tree::get_arrow() const& { return std::get<Arrow>(data->data); }
	inline Arrow&& Tree::get_arrow() && { return std::move(std::get<Arrow>(data->data)); }
	inline Arrow* Tree::get_if_arrow() { return std::get_if<Arrow>(&data->data); }
	inline Arrow const* Tree::get_if_arrow() const { return std::get_if<Arrow>(&data->data); }
...
inline expression_parser::path::Path path_of(Ts&&... ts) {
  ...
  inline bool is_child_valid(Tree const& in, std::uint64_t step) {
    return in.visit([&]<class T>(T part) -> bool {
      ...
      if constexpr(std::is_same_v<T, Lambda>) {
        ...
        case 1: return part.type.has_value();
  ...
  namespace detail {
    struct IteratorOutput {
      expression_parser::path::Path const& path;
      Tree& value;
    };
    class Iterator {
      expression_parser::path::Path path;
  ...
  namespace full_match {
	...
	inline bool operator==(Arrow const& lhs, Arrow const& rhs) {
	...
	inline bool operator==(Tree const& lhs, Tree const& rhs) {
		return lhs.visit(mdb::overloaded{
			...
			[&](Identifier const& component) {
				if(auto* other = rhs.get_if_identifier()) {
					return component == *other;
				} else {
					return false;
				}
			...
			[&](Arrow const& component) {
				if(auto* other = rhs.get_if_arrow()) {
					return component == *other;
				} else {
					return false;
				}
	...
	void format_indented(std::ostream& o, Tree const& value, std::size_t indent, Formatter&& format_inner = [](auto& o, auto const& v) { o << v; }) {
		...
		value.visit(mdb::overloaded{
			...
			[&](Lambda const& component) {
				...
				make_inner_indent(); o << ".type = "; if(component.type) { format_indented(o, *component.type, indent + 1, format_inner); o << "\n"; } else { o << "None\n"; }
				make_inner_indent(); o << ".arg_name = "; format_inner(o, component.arg_name); o << "\n";
				make_indent(); o << "}";
			},
			[&](Identifier const& component) {
				o << "Identifier {\n";
				make_inner_indent(); o << ".id = "; format_inner(o, component.id); o << "\n";
				make_indent(); o << "}";
			},
			[&](Hole const& component) {
				o << "Hole {\n";
			...
			[&](Arrow const& component) {
				o << "Arrow {\n";
				make_inner_indent(); o << ".domain = "; format_indented(o, component.domain, indent + 1, format_inner); o << "\n";
				make_inner_indent(); o << ".codomain = "; format_indented(o, component.codomain, indent + 1, format_inner); o << "\n";
				make_inner_indent(); o << ".arg_name = "; format_inner(o, component.arg_name); o << "\n";
				make_indent(); o << "}";
			}
		});
	...
	void format_indented(std::ostream& o, Tree const& value, std::size_t indent, Formatter&& format_inner = [](auto& o, auto const& v) { o << v; }) {
		...
		value.visit(mdb::overloaded{
			[&](Apply const& component) {
				...
				make_inner_indent(); o << ".lhs = "; format_indented(o, component.lhs, indent + 1, format_inner); o << "\n";
				make_inner_indent(); o << ".rhs = "; format_indented(o, component.rhs, indent + 1, format_inner); o << "\n";
				make_inner_indent(); o << ".position = "; format_inner(o, component.position); o << "\n";
				make_indent(); o << "}";
			},
			[&](Lambda const& component) {
				o << "Lambda {\n";
...
namespace expression_parser::located_output {
  ...
  {}
  inline Tree::Tree(Identifier arg):
    output([&]() -> expression_parser::output::Tree {
