...
namespace expression::pattern_path {
  struct Path {
...
namespace expression::pattern {
	...
	public:
		...
		template<class Visitor> requires (requires(Visitor&& visitor, Apply& apply, Fixed& fixed, Wildcard& wildcard) {
	std::forward<Visitor&&>(visitor)(std::forward<Apply&>(apply));
	std::forward<Visitor&&>(visitor)(std::forward<Fixed&>(fixed));
	std::forward<Visitor&&>(visitor)(std::forward<Wildcard&>(wildcard));
	typename std::common_type_t<std::invoke_result_t<Visitor&&, Apply&>, std::invoke_result_t<Visitor&&, Fixed&>, std::invoke_result_t<Visitor&&, Wildcard&> >;
		...
		template<class Visitor> requires (requires(Visitor&& visitor, Apply const& apply, Fixed const& fixed, Wildcard const& wildcard) {
	std::forward<Visitor&&>(visitor)(std::forward<Apply const&>(apply));
	std::forward<Visitor&&>(visitor)(std::forward<Fixed const&>(fixed));
	std::forward<Visitor&&>(visitor)(std::forward<Wildcard const&>(wildcard));
	...
	struct Tree::Impl {
		std::variant<Apply, Fixed, Wildcard> data;
	};
	
	inline Tree::Tree(Apply arg):data(mdb::in_place, std::move(arg)) {}
	inline Tree::Tree(Fixed arg):data(mdb::in_place, std::move(arg)) {}
	inline Tree::Tree(Wildcard arg):data(mdb::in_place, std::move(arg)) {}
	...
	inline Fixed& Tree::get_fixed() & { return std::get<Fixed>(data->data); }
	inline Fixed const& Tree::get_fixed() const& { return std::get<Fixed>(data->data); }
	inline Fixed&& Tree::get_fixed() && { return std::move(std::get<Fixed>(data->data)); }
	inline Fixed* Tree::get_if_fixed() { return std::get_if<Fixed>(&data->data); }
	inline Fixed const* Tree::get_if_fixed() const { return std::get_if<Fixed>(&data->data); }
	
	inline Wildcard& Tree::get_wildcard() & { return std::get<Wildcard>(data->data); }
	inline Wildcard const& Tree::get_wildcard() const& { return std::get<Wildcard>(data->data); }
	inline Wildcard&& Tree::get_wildcard() && { return std::move(std::get<Wildcard>(data->data)); }
	inline Wildcard* Tree::get_if_wildcard() { return std::get_if<Wildcard>(&data->data); }
	inline Wildcard const* Tree::get_if_wildcard() const { return std::get_if<Wildcard>(&data->data); }
...
template<class... Ts>
inline void path_append(expression::pattern_path::Path& path, Ts&&... ts) {
  (path.steps.push_back(path_segment_of(ts)) , ...);
}
template<class... Ts>
inline expression::pattern_path::Path path_of(Ts&&... ts) {
  expression::pattern_path::Path ret;
  ...
  inline Tree const& path_step(Tree const& in, std::uint64_t step) {
    ...
    inline Tree& path_lookup(Tree& in, expression::pattern_path::Path const& path) {
      auto* ret = &in;
    ...
    inline Tree const& path_lookup(Tree const& in, expression::pattern_path::Path const& path) {
      auto const* ret = &in;
      for(auto step : path.steps) {
        ret = &path_step(*ret, step);
      }
  ...
  namespace detail {
    struct IteratorOutput {
      expression::pattern_path::Path const& path;
      Tree& value;
    };
    class Iterator {
      expression::pattern_path::Path path;
  ...
  namespace match_result {
    ...
    template<class Index> Fixed(Index&&) -> Fixed<Index>;
  ...
  namespace match {
    ...
    template<class Lhs, class Rhs> Apply(Lhs, Rhs) -> Apply<Lhs, Rhs>;
      ...
      auto match(Tree& tree) const {
        auto& component = tree.get_fixed();
        return match_result::Fixed{
      ...
      auto match(Tree const& tree) const {
        auto const& component = tree.get_fixed();
        return match_result::Fixed{
            .index = component.index
        };
      }
      ...
      auto match(Tree& tree) const {
        auto& component = tree.get_wildcard();
        return match_result::Wildcard{
        };
      }
      auto try_match(Tree& tree) const {
        using MatchType = decltype(match(tree));
      ...
      auto match(Tree const& tree) const {
        auto const& component = tree.get_wildcard();
        return match_result::Wildcard{
        };
      }
      auto try_match(Tree const& tree) const {
  ...
  namespace full_match {
    ...
    struct Fixed {
      Index index;
      bool check(Tree const& tree) const {
        if(auto const* component = tree.get_if_fixed()) {
	...
	inline bool operator==(Fixed const& lhs, Fixed const& rhs) {
		return lhs.index == rhs.index;
	}
	inline bool operator==(Wildcard const& lhs, Wildcard const& rhs) {
		return true;
	...
	inline bool operator==(Tree const& lhs, Tree const& rhs) {
		return lhs.visit(mdb::overloaded{
			...
			[&](Fixed const& component) {
				if(auto* other = rhs.get_if_fixed()) {
					return component == *other;
				} else {
					return false;
				}
			},
			[&](Wildcard const& component) {
				if(auto* other = rhs.get_if_wildcard()) {
					return component == *other;
				} else {
					return false;
				}
